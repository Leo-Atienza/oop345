...
namespace seneca {
	...
	public:
		...
		int getAttackAmnt() const override { 
			...
			double atk = static_cast<double>(m_baseAttack) + 2.0 * static_cast<double>(m_weapon);
		...
		void takeDamage(int dmg) override {
			...
			m_firstAbility.transformDamageReceived(reduced);
			m_secondAbility.transformDamageReceived(reduced);
